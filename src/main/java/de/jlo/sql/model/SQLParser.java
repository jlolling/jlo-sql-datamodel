...
import de.jlo.sql.generator.StringReplacer;
...
public final class SQLParser {
    ...
    public String parseScript(String text, int textOffset) {
    	if (logger.isDebugEnabled()) {
    		logger.debug("Start parsing script...");
    	}
        initParser();
    	if (text == null) {
                ...
                for (; i < text.length(); i++) {
                    ...
                    if (inStringConstant == false && inLineComment == false && inBlockComment == false) {
                        if (c == '\'') { // String-Konstante ??
                            ...
                            temp.append(' ');
                            temp.append(' ');
                        } else if (c == ';' || isScriptEnd(c0, c, c1)) { // Ende des Statements gefunden ?
                        	if (c == ';') {
                        		if (plsql) {
                                    temp.append(c);
                        		} else {
                            ...
                            } else if (plsql && isScriptEnd(c0, c, c1)) {
                        		endOfStatement = true;
                            }
                        	if (i == text.length() - 1) {
                        		endOfStatement = true;
                        	...
                        	if (endOfStatement) {
                                if (isPreparedStatement && plsql == false) {
                                	...
                                	if (sqlStr.isEmpty() == false) {
                                        ...
                                        parsedStatements.add(sql);
                                        if (logger.isDebugEnabled()) {
                                        	logger.debug("add statement (" + parsedStatements.size() + ") " + " code=" + sql.getSQL());
                                        }
                                	} else {
                                		if (logger.isDebugEnabled()) {
                                			logger.debug("skip empty statement");
                                		}
                                	}
                                } else {
                                	String sqlStr = temp.toString().trim();
                            ...
                            if (endOfStatement) { // vorheriges Statement-Ende gefunden,
                                ...
                                if (Character.isWhitespace(c) == false && isScriptEnd(c0, c, c1) == false && c != ';') {
    ...
    private static boolean isScriptEnd(char c0, char c, char c1) {
    	return (c0 == '\n' && c == '/' && (Character.isWhitespace(c1)));
    }
    ...
    public void printDebugData() {
        for (int i = 0; i < parsedStatements.size(); i++) {
            System.out.println(parsedStatements.get(i));
        }
    }
    ...
    public static String getTableForQuery(String sql) {
        ...
        while (p0 < sql.length()) {
            ...
            if (c == '(') {
                ...
                if (inWord) {
                    if (sb.toString().toLowerCase().equals("from")) {
