...
import de.jlo.sql.ext.DatabaseExtensionFactory;
...
public final class SQLDataModel extends SQLObject implements Comparable<SQLDataModel> {
	...
	public SQLDataModel(Connection connection) throws SQLException {
		...
		connectionDriverName = connection.getMetaData().getDriverName();
		databaseExtension = DatabaseExtensionFactory.getDatabaseExtension(connectionDriverName);
		...
		try {
			...
			if (dbmd != null) {
				...
				while (rsSchemas.next()) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("In catalog: " + catalog.getName() + " schema found: " + name);
					}
