...
import de.jlo.sql.generator.SQLCodeGenerator;
...
//    private List<SQLSequence> sequencesToChange = new ArrayList<SQLSequence>();
    ...
    private boolean compareFieldsTolerant = true;
    ...
    public void compare(Connection sourceConnection, String sourceSchemaName, Connection targetConnection, String targetSchemaName) throws Exception {
    	SQLDataModel sourceModel = new SQLDataModel(sourceConnection);
    	SQLDataModel targetModel = new SQLDataModel(targetConnection);
    	SQLSchema sourceSchema = sourceModel.getSchema(sourceSchemaName);
    	if (sourceSchema == null) {
    		throw new Exception("Could not find the source schema: " + sourceSchemaName);
    	}
    	SQLSchema targetSchema = targetModel.getSchema(targetSchemaName);
    	if (targetSchema == null) {
    		throw new Exception("Could not find the target schema: " + targetSchemaName);
    	}
    ...
    private void compare(SQLField field1, SQLField field2) {
    	...
    	if (compareFieldsTolerant) {
        	...
        	different = (fd1.equals(fd2) == false); 
    	}
    	if (different) {
	...
	public boolean isCompareFieldsTolerant() {
		return compareFieldsTolerant;
	}

	public void setCompareFieldsTolerant(boolean compareFieldsTolerant) {
		this.compareFieldsTolerant = compareFieldsTolerant;
	}
